...
extern  __IO uint32_t TIM2_TimingMillis;
...
extern volatile uint8_t gRxBuf[64];
...
extern volatile uint32_t ISusartDataDomming;
    ...
    if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET){
          ...
          u8InChar = USART_ReceiveData(USART1) & 0xFF;
			  ...
			  if (!begin){
					...
					if (u8InChar == 0x1B){
						begin = 1;
						ISusartDataDomming = 1;
					}
			  }
			  if (begin){
					gpu8RxBuf[gu32RxSize] = u8InChar;
					 //jadge gu32RxSize whether is bigger than 64 bytes. if it is,		
		         	if(gu32RxSize != 299){
		           		gu32RxSize++; 
		         	}
	          }
